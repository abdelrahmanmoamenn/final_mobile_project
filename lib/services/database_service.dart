...
class DatabaseService {
  ...
  Future<bool> _trySyncLoggedSet(Map<String, dynamic> data) async {
    ...
    // CRITICAL: Stop sync if user doesn't match current login
    if (currentUser == null || data['userId'] != currentUser.uid) return false;
  ...
  Future<void> syncPendingOperations() async {
    ...
    try {
      ...
      for (var op in pending) {
        ...
        // SILENTLY SKIP AND MARK AS SYNCED if it belongs to someone else
        // This stops the repeated "Permission Denied" errors in logs.
  ...
  Future<int> insertLoggedSet({
    ...
    await queuePendingOperation(tableName: 'logged_sets', operation: 'INSERT', data: data);
  ...
  Future<void> updatePersonalRecord({
    ...
    if (existing.isNotEmpty) {
      ...
      if (weight > existingWeight) {
        ...
        await db.update('personal_records', dataToSync, where: 'userId = ? AND exerciseName = ?', whereArgs: [userId, exerciseName]);
      }
    } else {
      dataToSync = {'userId': userId, 'exerciseName': exerciseName, 'weight': weight, 'reps': reps, 'lastUpdated': now};
      await db.insert('personal_records', dataToSync);
    }

    if (dataToSync != null) {
      await queuePendingOperation(tableName: 'personal_records', operation: 'UPDATE', data: dataToSync);
      syncPendingOperations();
    }
  }
  ...
  Future<int> startWorkoutSession(String userId) async {
    ...
    await queuePendingOperation(tableName: 'workout_sessions', operation: 'INSERT', data: {...data, 'id': id});
    syncPendingOperations();
    return id;
  }
  ...
  Future<void> endWorkoutSession(String userId) async {
    final db = await database;
    final now = DateTime.now().toIso8601String();
    final sessions = await db.query('workout_sessions', where: 'userId = ? AND endTime IS NULL', whereArgs: [userId], orderBy: 'startTime DESC', limit: 1);

    if (sessions.isNotEmpty) {
      final session = sessions.first;
      final sessionId = session['id'] as int;
      final startTime = session['startTime'] as String;
      final setsResult = await db.rawQuery('SELECT COUNT(*) as count FROM logged_sets WHERE userId = ? AND date >= ?', [userId, startTime]);
      ...
      await queuePendingOperation(tableName: 'workout_sessions', operation: 'UPDATE', data: fullData);
      syncPendingOperations();
    }
  }
  ...
  Future<int> getStreak(String userId) async {
    ...
    for (var row in rows) {
      ...
      else if (row['day'] == _dateKey(check.subtract(const Duration(days: 1)))) {
        streak++; check = DateTime.parse(row['day'] as String).subtract(const Duration(days: 1));
      } else break;
    }
    return streak;
  }
  ...
  Future<void> restoreUserData(String userId) async {
    try {
      final db = await database;
      final prSnapshot = await _firebaseRef.child('users/$userId/personal_records').get();
      if (prSnapshot.exists) {
        final data = prSnapshot.value as Map<dynamic, dynamic>;
        for (var entry in data.values) {
          final record = Map<String, dynamic>.from(entry as Map);
          await db.insert('personal_records', record, conflictAlgorithm: ConflictAlgorithm.replace);
        }
      }
      final setsSnapshot = await _firebaseRef.child('users/$userId/logged_sets').get();
      if (setsSnapshot.exists) {
        final data = setsSnapshot.value as Map<dynamic, dynamic>;
        for (var entry in data.values) {
          final set = Map<String, dynamic>.from(entry as Map);
          final existing = await db.query('logged_sets', where: 'userId = ? AND date = ? AND exerciseId = ? AND setNumber = ?', whereArgs: [set['userId'], set['date'], set['exerciseId'], set['setNumber']]);
          if (existing.isEmpty) await db.insert('logged_sets', set);
        }
      }
      final sessionsSnapshot = await _firebaseRef.child('users/$userId/sessions').get();
      if (sessionsSnapshot.exists) {
        final data = sessionsSnapshot.value as Map<dynamic, dynamic>;
        for (var entry in data.values) {
          final session = Map<String, dynamic>.from(entry as Map);
          final existing = await db.query('workout_sessions', where: 'userId = ? AND startTime = ?', whereArgs: [session['userId'], session['startTime']]);
          if (existing.isEmpty) await db.insert('workout_sessions', session);
        }
      }
    } catch (e, stack) { appError('DatabaseService.restoreUserData', e, stack); }
  }
